...
class _MyAppState extends State<AppWithNavigation> {
  ...
  Future<void> saveFavorites(Set<int> favorites) async {
    ...
    await prefs.setStringList(
        'favorites', favorites.map((id) => id.toString()).toList());
  }

  Future<void> savePanier(Set<int> panier) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(
        'panier', panier.map((id) => id.toString()).toList());
  }
  ...
  Future<Set<int>> loadFavorites() async {
    ...
    return storedFavorites != null
        ? storedFavorites.map((id) => int.parse(id)).toSet()
        : {};
  }
  ...
  Future<Set<int>> loadPanier() async {
    ...
    return storedPanier != null
        ? storedPanier.map((id) => int.parse(id)).toSet()
        : {};
  }
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<List<Article>> fetchArticles() async {
    ...
    if (response.statusCode == 200) {
      ...
      _allArticles =
          jsonResponse.map((article) => Article.fromJson(article)).toList();
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () =>
                                  widget.toggleFavorite(article.id),
                            ),
                            IconButton(
